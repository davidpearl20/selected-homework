...
/*creates a buffer for each step of this process*/
void encode(char* message, char* key, char* encoded, int length)
{
    //create an integer representation of the message
    ...
    //create an integer representation of message + key
    int message_plus_key[length];
    //get the integer representation of the message + key
    for (int i = 0; i < length; i++)
        message_plus_key[i] = message_ints[i] + key_ints[i];

    //convert message + key to encoded

    for (int i = 0; i < length; i++)
    {
        encoded[i] = (char)(message_plus_key[i] % 27 + 65);
        if (encoded[i] == '[')//quirk of the ascii math, manually inputting space character
            encoded[i] = ' ';
    }
}
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        if (pid == 0)
        {
            ...
            charsRead = recv(connectionSocket, buffer, SIZE - 1, MSG_WAITALL);
            if (charsRead < 0) {
            ...
            if (strcmp(password, "enc_client") != 0)
            {
                fprintf(stderr, "attempting to make an unauthorized connection, will only accept enc_client");
                exit(2);
            }
            ...
            encode(message, key, encoded, strlen(message));
